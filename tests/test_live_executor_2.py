...
class LiveExecutor2TtpTests(unittest.TestCase):
    ...
    def test_wait_same_side_imba_only_switches_order_set_to_ttp(self) -> None:
        ...
        with (
            patch.dict("os.environ", {"LIVE_WAIT_TO_TTP_ON_SAME_SIDE_IMBA": "1"}, clear=False),
            patch("quant.execution.live_executor_2.get_live_gate_state", return_value={"gate_on": 1, "gate_countertrend_on": 1, "gate_trend_on": 0}),
            patch("quant.execution.live_executor_2._load_renko_bars", return_value=pd.DataFrame({"ts": pd.to_datetime(["2026-03-30T12:00:00Z"], utc=True), "open": [100.0], "high": [100.0], "low": [100.0], "close": [100.0]})),
            patch("quant.execution.live_executor_2._load_signals_df", return_value=pd.DataFrame()),
    ...
    def test_wait_same_side_imba_does_not_switch_to_ttp_by_default(self) -> None:
        state = ExecutorState(
            latched_exit_engine="flip",
            open_leg_mode="tp2",
            open_leg_side="short",
        ...
        self.assertEqual(oms.enter_market_calls, [])
        self.assertEqual(oms.flatten_market_calls, [])
        ttp_exit_calls = [call for call in oms.stop_exit_calls if call[4] == "ttp_exit"]
        self.assertEqual(ttp_exit_calls, [])
        ttp_flip_calls = [call for call in oms.stop_entry_calls if call[4] in ("ttp_flip_entry_short", "ttp_flip_entry_long")]
        self.assertEqual(ttp_flip_calls, [])
    ...
    def test_flat_after_ttp_keeps_matching_flip_entry_when_still_open(self) -> None:
        state = ExecutorState(latched_exit_engine="flip", last_live_side="long")
        broker = _LiveBroker(pos=0.0, bid=100.0, ask=100.0)
        oms = _LiveOms()
        oms.open_orders = {
            "ttp_flip_entry_short": {"stop_price": 98.901, "reduce_only": False},
    ...
    def test_flat_after_ttp_does_not_keep_flip_entry_when_already_gone(self) -> None:
        state = ExecutorState(latched_exit_engine="flip", last_live_side="long")
        broker = _LiveBroker(pos=0.0, bid=100.0, ask=100.0)
        oms = _LiveOms()
        ...
        self.assertNotEqual(state.last_action, "hold_ttp_flip_follow_entry")
        self.assertIn(("SOL-USDT", "ttp_flip_entry_short"), oms.cancel_calls)
        self.assertIn(("SOL-USDT", "ttp_flip_entry_long"), oms.cancel_calls)
    ...
    def test_manual_live_long_cancels_stale_ttp_short_follow_entry_when_terminal_is_flat(self) -> None:
        state = ExecutorState(latched_exit_engine="flip", last_live_side="long")
        broker = _LiveBroker(pos=5.0, bid=100.0, ask=100.0)
        oms = _LiveOms()
        oms.open_orders = {
        ...
        self.assertEqual(state.last_action, "hold_manual_live_position")
        self.assertEqual(state.flat_latch_reason, "manual_live_position")
        self.assertIsNotNone(state.flat_until_new_signal_ts)
        self.assertIn(("SOL-USDT", "ttp_flip_entry_short"), oms.cancel_calls)
        self.assertIn(("SOL-USDT", "ttp_flip_entry_long"), oms.cancel_calls)
        self.assertEqual(oms.enter_market_calls, [])
        self.assertEqual(oms.flatten_market_calls, [])
    ...
    def test_manual_observe_only_latch_ignores_stale_short_terminal_until_fresh_signal(self) -> None:
        state = ExecutorState(
            latched_exit_engine="flip",
            last_live_side="long",
            flat_until_new_signal_ts="2026-03-30T12:00:00+00:00",
        ...
        oms.open_orders = {
            ...
            "flat_entry_short": {"stop_price": 99.0, "reduce_only": False},
        }
        ...
        with (
            ...
            patch("quant.execution.live_executor_2._load_renko_bars", return_value=pd.DataFrame({"ts": pd.to_datetime(["2026-03-30T12:00:00Z"], utc=True), "open": [100.0], "high": [100.0], "low": [100.0], "close": [100.0]})),
            patch("quant.execution.live_executor_2._load_signals_df", return_value=pd.DataFrame()),
            patch("quant.execution.live_executor_2._latest_backtest_event", return_value=({"ts": "2026-03-30T12:00:00Z", "event": "signal_flip_exit", "side": -1, "seq": 1}, {"mode": "TTP", "pos": -1, "side": "short", "ttp": 101.0, "entry_px": 100.0, "entry_bar_ts": "2026-03-30T11:00:00Z"})),
            patch("quant.execution.live_executor_2.get_latest_imba_barriers", return_value={"ts": None, "long_barrier": 101.0, "short_barrier": 99.0}),
            patch("quant.execution.live_executor_2._latest_signal", return_value={"ts": pd.Timestamp("2026-03-30T12:00:00Z"), "signal": -1}),
            patch("quant.execution.live_executor_2.write_execution_state", return_value={}),
            patch("quant.execution.live_executor_2._write_dashboard_levels", return_value=None),
    ...
    def test_stale_flip_latch_does_not_reclassify_open_tp2_leg_into_ttp(self) -> None:
        state = ExecutorState(
            latched_exit_engine="flip",
            open_leg_mode="tp2",
            open_leg_side="long",
        ...
        with (
            ...
            patch("quant.execution.live_executor_2._load_renko_bars", return_value=pd.DataFrame({"ts": pd.to_datetime(["2026-03-30T12:00:00Z"], utc=True), "open": [100.0], "high": [100.0], "low": [100.0], "close": [100.0]})),
            patch("quant.execution.live_executor_2._load_signals_df", return_value=pd.DataFrame()),
            patch("quant.execution.live_executor_2.read_execution_state", return_value={"side": "long", "position": 5.0, "mode": "TP2", "exit_engine": "tp2", "entry_px": 100.0}),
            patch("quant.execution.live_executor_2._latest_backtest_event", return_value=({"ts": "2026-03-30T12:00:00Z", "event": "tp_exit", "side": -1, "seq": 1}, {"mode": "TTP", "pos": -1, "side": "short", "ttp": 101.0, "entry_px": 100.0, "entry_bar_ts": "2026-03-30T11:00:00Z"})) as latest_bt,
            patch("quant.execution.live_executor_2.run_follow_tp2_state_machine", return_value=(pd.DataFrame(), pd.DataFrame([{"ts": "2026-03-30T12:00:00Z", "event": "tp2_live", "side": 1, "seq": 1}]), {"mode": "TP2", "pos": 1, "side": "long", "sl": 95.0, "tp1": 104.0, "tp2": 108.0, "entry_px": 100.0, "entry_bar_ts": "2026-03-30T11:00:00Z"})) as run_tp2,
            patch("quant.execution.live_executor_2.get_latest_imba_barriers", return_value={"ts": None, "long_barrier": 101.0, "short_barrier": 99.0}),
            patch("quant.execution.live_executor_2.write_execution_state", return_value={}),
            patch("quant.execution.live_executor_2._write_dashboard_levels", return_value=None),
            patch("quant.execution.live_executor_2._append_action_event", return_value=None),
        ...
        latest_bt.assert_not_called()
        run_tp2.assert_called_once()
...
class KrakenNativeStopSyncTests(unittest.TestCase):
    ...
    def test_sync_kraken_stop_loss_does_not_use_ttp_fallback_in_wait_mode(self) -> None:
        broker = _NativeStopBroker(
            pos=4.8,
            open_stop_orders=[
