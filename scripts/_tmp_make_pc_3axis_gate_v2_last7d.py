...
PRED_PATH = Path("data/runs/PC_RENKO_last7d_box003/pc_v02_renko/predictions.parquet")
...
def main() -> None:
    ...
    if cut <= 0 or cut >= n:
        raise ValueError(f"invalid TRAIN_FRAC={TRAIN_FRAC} for n={n}")
    train = slice(0, cut)
    ...
    # Short-horizon tuned:
    # slightly stricter on instability / elasticity, a bit earlier on drift
    t_instab_35 = q_train(instab, train, 0.35)
    t_elas_25 = q_train(elas, train, 0.25)
    t_drift_55 = q_train(np.abs(drift_eff), train, 0.55)

    # Looser short-horizon variant
    t_instab_30 = q_train(instab, train, 0.30)
    t_elas_20 = q_train(elas, train, 0.20)
    t_drift_50 = q_train(np.abs(drift_eff), train, 0.50)

    g1, g2, g3, base_2of3, base_3of3 = gates(
        instab, elas, drift_eff,
        t_instab_35, t_elas_25, t_drift_55
    )
    _, _, _, loose_2of3, loose_3of3 = gates(
        instab, elas, drift_eff,
        t_instab_30, t_elas_20, t_drift_50
    )
    ...
    out = pd.DataFrame(
        {
            ...
            "gate_base_instab_q35": g1,
            "gate_base_elas_q25": g2,
            "gate_base_drift_abs_q55": g3,
    ...
    print(
        "thresholds_base:",
        {
            "instab_q35": t_instab_35,
            "elas_q25": t_elas_25,
            "drift_abs_q55": t_drift_55,
        },
    )
    print(
        "thresholds_loose:",
        {
            "instab_q30": t_instab_30,
            "elas_q20": t_elas_20,
            "drift_abs_q50": t_drift_50,
        },
    )
    print(
        "ON-rate base_2of3/base_3of3/loose_2of3/loose_3of3:",
