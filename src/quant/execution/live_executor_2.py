...
def run_once(
    *,
    ...
    log.info(
        "executor sizing: equity=%.2f pos_pct=%.2f leverage=%.1f mid=%.4f mult=%.4f -> qty=%.4f",
    ...
    if gate_changed and current_side != "flat":
        want_side = None
        action = "exit_long" if current_side == "long" else "exit_short"
    ...
    if gate_changed and current_side != "flat":
        event_name = "gate_change_flatten"
    ts_iso = _now_iso()
